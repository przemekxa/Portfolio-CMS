...
import { Box, Card, CardActions, CardMedia, Grid, Stack, Typography } from "@mui/material";
...
const SectionData: React.FC<Props> = ({ section }) => {
  if(isPageHeader(section)) {
    return <Stack style={{fontStyle: "italic"}}>
      <p>Displays the title of the page.</p>
      <p>Shows the page card: <b>{section.showCard ? "Yes" : "No"}</b></p>
    </Stack>
  }
  ...
  if(isSubpageCarousel(section)) {
    return <p style={{fontStyle: "italic"}}>Displays subpages of this page in form of a carousel.</p>
  }

  if(isContact(section)) {
    return <Box display="grid" gridTemplateColumns="auto 1fr" columnGap={2} rowGap={2}>
